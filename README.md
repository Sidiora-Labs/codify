...
### The agent panel (VS Code)

The extension drives the same machinery one task at a time — through a Codify-native panel. **Start Agent Session on Task** claims the task, seeds a prompt from `cg resume --task <id> --prompt`, and opens the **agent panel**: the extension is an [Agent Client Protocol](https://agentclientprotocol.com) client that spawns Claude Code or Codex through its ACP adapter (`claude-code-acp` / `codex-acp`, or any ACP agent via `codify.acp.customCommand`) and renders the session beside the editor — streamed replies and thinking, tool calls as live status cards with diffs, the agent's plan, and permission requests as inline buttons. Every panel session gets **Codify's MCP server injected automatically** (`cg mcp`), so the agent has the graph, spec, and memory tools with zero per-repo config; the agent's ACP file reads and writes are served by the extension and confined to the workspace. Set `codify.agent.interface: "terminal"` for the classic seeded terminal instead — that path is also offered automatically when the adapter fails to start.
