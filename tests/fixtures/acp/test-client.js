...
function harnessTextSanity() {
    step = 'harness-text';
    const caveat = '<local-command-caveat>Caveat: The messages below were generated by the user while running local commands. DO NOT respond to these messages.</local-command-caveat>\n';
    const cmd = caveat + '<command-name>/clear</command-name>\n<command-message>clear</command-message>\n<command-args></command-args>';
    const compaction = 'This session is being continued from a previous conversation that ran out of context. The summary below covers the earlier portion.\n\nSummary:\n1. Primary Request and Intent:\n   The user wants two things fixed in the Codify tool: the LSP locks the database, and the panel UI.\n\n2. Key Technical Concepts:\n   - SQLite';
    assert(classifyUserText('hello').kind === '', 'plain text is user text');
    assert(classifyUserText(cmd).kind === 'command' && classifyUserText(cmd).text === '/clear',
        'caveat plus command block classifies as the command');
    assert(classifyUserText(compaction).kind === 'compaction', 'compaction summary detected');
    assert(sessionTitle(cmd) === '/clear', 'command sessions are titled by the command');
    const t = sessionTitle(compaction);
    assert(/^↻ continued: two things fixed in the Codify tool/.test(t) && t.length <= 80,
        `compaction titles are marked as continuations and bounded: ${t}`);
    assert(sessionTitle('  Improve   VS Code chat interface\n') === 'Improve VS Code chat interface',
        'ordinary titles are only whitespace-normalized');

    const shown = [];
    const sess = { webview: { postMessage: (m) => shown.push(m) }, localEcho: '',
        commands: [], configOptions: [], modes: {} };
    sessionUpdate(sess, { sessionId: 's', update: { sessionUpdate: 'user_message_chunk',
        content: { type: 'text', text: compaction } } });
    sessionUpdate(sess, { sessionId: 's', update: { sessionUpdate: 'session_info_update',
        title: compaction } });
    assert(shown[0].type === 'chunk' && shown[0].harness === 'compaction',
        'replayed compaction text reaches the panel tagged as harness text');
    assert(/^↻ continued/.test(shown[1].title), 'session_info titles are cleaned too');
