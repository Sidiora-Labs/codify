...
- **The agent panel** (`acp.js`, `agentpanel.html`): a zero-dependency Agent Client Protocol (v1) client that spawns Claude Code or Codex through its ACP adapter and renders the session in a Codify-native webview — streamed replies and collapsed thinking, tool-call cards with diffs, the agent's plan, permission requests as inline buttons, and Stop as `session/cancel`
