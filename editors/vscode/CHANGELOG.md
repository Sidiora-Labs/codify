...
_Generated by Codify 0.4.0 from local snapshots (`cg log`). Symbol-level changes are derived from the code graph._

## 2026-09-25 — Codify 0.10.0 · extension 1.3.0

The task board becomes a task tree you can work a fleet from.

- Tasks group by feature → section → wave, the way the spec is written and the way a fleet divides work; each wave names the branch its worker takes
- Every row carries what decides whether you can pick it up: status icon, the agent holding the lease and its role, the branch, and the requires that are not met yet (`blocked by 3.2`)
- Filter the tree by status, wave, or owner, and search it by id, title, section, owner, symbol, or path; the active filter is shown in the view title with a count badge and survives a window reload
- A task detail panel (click any task) shows its acceptance criteria, do-steps, declared symbols with their resolved location and reference count, touched paths with whether a matching change exists, the verify command, the tagged commits, and the memories written under it — clicking a resolved symbol opens it, an unresolved one runs `cg symbol`
...
## 2026-09-25 — Codify 0.10.0 · extension 1.2.8

The board no longer competes with the agents for the CPU.

- Every refresh goes through one scheduler (`refresh.js`): a single chain of `cg` calls at a time, bursts debounced into one run, a two-second floor between runs unless a command asked for one, and at most one trailing run queued behind a chain in flight
- Each chain syncs once with a three-second freshness window at background priority and never waits on another process's pass; `spec trace --no-sync` then answers from that index instead of walking the tree again
...
- Replayed sessions no longer show harness text as user bubbles: Claude Code's local-command caveat is dropped, `<command-name>` blocks become a one-line note, and context-compaction summaries collapse into a labelled note that still opens
- Past-session titles are cleaned: harness prefixes stripped, `/clear` sessions titled by the command, and sessions that start at a compaction point titled `↻ continued: …` from the summary's primary request
- The tool timeline stays a single row; older calls fold into a `+N` count instead of wrapping over the toolbar

## 2026-09-04 — Codify 0.8.1 · extension 1.1.0

The Agent view is rebuilt around transparency: what runs now, what each call did, and whether the task qualified.

- A provider picker labels Codex, Claude Code, and a Custom ACP command by name, remembers the choice per workspace, and a gear quick pick (`Codify: Configure Agent Provider`) switches provider or edits each adapter's launch command in place
- A Codify toolbar above the transcript runs brief, next, status, review, check, guard, changes, and remember with one click; implemented, done, and handoff appear when a task is attached, and `done` runs the real qualification with its result card marked green or red
- Tool cards show a kind badge and colour, status tick, elapsed time, and the raw input; sub-agents render as their own cards with the delegated prompt and nested child tools
- A per-turn tool timeline, a now line naming the call in flight, and a turn summary pill (outcome, tools, failures, sub-agents, files touched, duration) keep the work legible without opening cards
- Context usage draws as a bar; cards and messages ease in, the live dot pulses while working, and every animation respects reduced motion
- `/guard`, `/done`, and `/implemented` join the slash palette; the DOM fixture covers provider labels, the toolbar, sub-agent nesting, the timeline, and turn summaries
